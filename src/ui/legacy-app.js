import { loadCuesForVideo, saveCuesForVideo, loadGlobalMappings, saveGlobalMappings } from "../storage/storage.js";
import { createWorkletLoader } from "../worklets/worklet-loader.js";
import { ActionTypes } from "../store/actions.js";

// ---- Basic helpers (added by ChatGPT fix) ----
const YTBM_ROOT_ID = "ytbm-root";
let sessionAbortSignal = null;
const workletLoader = createWorkletLoader();

function getRootContainer() {
  let root = document.getElementById(YTBM_ROOT_ID);
  if (!root) {
    root = document.createElement("div");
    root.id = YTBM_ROOT_ID;
    ...
    if (deviceId && deviceId !== 'default') {
      ...
      if (!canUseCtxSink || !success) {
        ...
        try {
          ...
          getRootContainer().appendChild(outputAudio);
...
function setupPulseShowWatcher() {
  addTrackedListener(document, "mousemove", stopPulseShowYTControls);
}
...
controlRow.appendChild(bpmInput);
    ...
    getRootContainer().appendChild(touchPopup);
  ...
  function makeOverlayDraggable(overlay, handle) {
    ...
    addTrackedListener(document, "mousemove", e => { if (!dragging) return; overlay.style.left = (e.clientX - offsetX) + "px"; overlay.style.top = (e.clientY - offsetY) + "px"; });
    addTrackedListener(document, "mouseup", () => { dragging = false; document.body.style.userSelect = ""; });
  }
...
function triggerPadCue(padIndex) {
  ...
  // hideYouTubePopups is called during bootstrap
...
function setupDigitCueShortcuts() {
  addTrackedListener(
    document,
    "keydown",
    (e) => {
...
function setupStartupListeners() {
  ...
  if (document.readyState === "loading") {
    addTrackedListener(document, 'DOMContentLoaded', onReady);
  } else {
    onReady();
  }
  addTrackedListener(document, 'click', () => {
    ensureAudioContext();
  }, { once: true });
}

function setupCueModifierShortcuts() {
...
function normalizeListenerOptions(options) {
  if (!options) return sessionAbortSignal ? { signal: sessionAbortSignal } : undefined;
  if (typeof options === "boolean") {
    return sessionAbortSignal ? { capture: options, signal: sessionAbortSignal } : options;
  }
  if (options.signal || !sessionAbortSignal) return options;
  return { ...options, signal: sessionAbortSignal };
}
...
function addTrackedListener(target, type, listener, options) {
  ...
  cleanupFunctions.push(() => target.removeEventListener(type, listener, normalized));
}

function reportSessionError(message, error) {
  if (!appStore) return;
  appStore.dispatch({
...
function registerObserver(observer) {
  sessionContext?.registerObserver?.(observer);
  return observer;
}
function cleanupResources() {
  ...
  const root = document.getElementById(YTBM_ROOT_ID);
  if (root) {
    root.remove();
  }
...
function mountMinimalUIContainer() {
  ...
  if (!minimalUIContainer.isConnected) {
    getRootContainer().appendChild(minimalUIContainer);
  }
...
function generateSimpleReverbIR(ctx) {
  ...
  async function createCassetteNode(ctx) {
    ...
    try {
      await workletLoader.load(ctx, "worklets/cassette-processor-worklet.js");
    } catch (error) {
      reportSessionError("Cassette worklet failed to load.", error);
  ...
  async function createLoopRecorderNode(ctx) {
    ...
    try {
      await workletLoader.load(ctx, "worklets/loop-recorder-worklet.js");
    } catch (error) {
      reportSessionError("Loop recorder worklet failed to load.", error);
      return ctx.createGain();
    }
  ...
  async function createVinylBreakNode(ctx) {
    try {
      await workletLoader.load(ctx, "worklets/vinyl-break-worklet.js");
    } catch (error) {
      reportSessionError("Vinyl break worklet failed to load.", error);
      return ctx.createGain();
    }
  ...
  async function createStutterNode(ctx) {
    try {
      await workletLoader.load(ctx, "worklets/stutter-proc-worklet.js");
    } catch (error) {
      reportSessionError("Stutter worklet failed to load.", error);
      return ctx.createGain();
    }
  ...
  async function createPhaserNode(ctx) {
    try {
      await workletLoader.load(ctx, "worklets/phaser-proc-worklet.js");
    } catch (error) {
      reportSessionError("Phaser worklet failed to load.", error);
      return ctx.createGain();
    }
...
async function createVinylBreakEffect(ctx){
  const node=await createVinylBreakNode(ctx); return {in:node, out:node, update(x,y){ if (node?.port) node.port.postMessage({speed:0.2+0.8*(1-y)}); }};
}
...
async function createPhaserEffect(ctx){
  const node=await createPhaserNode(ctx); return {in:node,out:node,update(){ if (node?.port) node.port.postMessage({}); }};
}
...
function createJagFilterEffect(ctx){
  ...
  async function createBitDecimatorEffect(ctx){
    try {
      await workletLoader.load(ctx, "worklets/bit-dec-worklet.js");
    } catch (error) {
      reportSessionError("Bit decimator worklet failed to load.", error);
      const passthrough = ctx.createGain();
      return { in: passthrough, out: passthrough, update() {} };
    }
  ...
  async function createTwelveBitEffect(ctx){
    try {
      await workletLoader.load(ctx, "worklets/bit12-proc-worklet.js");
    } catch (error) {
      reportSessionError("12-bit worklet failed to load.", error);
      const passthrough = ctx.createGain();
      return { in: passthrough, out: passthrough, update() {} };
    }
...
function buildSidechainWindow() {
  ...
  getRootContainer().appendChild(sidechainWindowContainer);
  ...
  if (!window._ytbmSidechainMouseBound) {
    addTrackedListener(document, 'mouseup', stopSidechainDraw);
...
function importMedia() {
  ...
  input.addEventListener("change", (e) => {
    ...
    if (media) {
      ...
      getRootContainer().appendChild(customVideo);
...
function setupProgressBarListeners() {
  ...
  addTrackedListener(document, "fullscreenchange", () => {
  ...
  if (video) {
    addTrackedListener(video, "loadeddata", () => {
...
function createOrUpdateVideoPreviewElement() {
  if (!videoPreviewElement) {
    ...
    getRootContainer().appendChild(videoPreviewElement);
...
async function onImportAudioClicked() {
  ensureAudioContext().then(async () => {
    ...
    try {
      ...
      input.style.display = "none";
      getRootContainer().appendChild(input);
...
function setupVideoCueListener() {
  ...
  addTrackedListener(video, "loadedmetadata", loadCuePointsAtStartup);
}

async function loadCuePointsAtStartup() {
  const vid = getVideoElement();
  if (!vid || !vid.duration) return;
  ...
  if (vidID) {
    try {
      const stored = await loadCuesForVideo(
        vidID,
        vid.duration,
        (message, error) => reportSessionError(message, error)
      );
      Object.assign(cuePoints, stored);
    } catch (e) {
      console.warn("Error loading stored cues:", e);
    }
  }
...
function saveCuePointsToURL() {
  ...
  if (vidID) {
    void saveCuesForVideo(vidID, cuePoints);
  }
}
...
function exportLoop() {
  ensureAudioContext().then(() => {
    if (videoLooperState !== "idle" && videoPreviewURL) {
      ...
      getRootContainer().appendChild(a);
      a.click();
...
function addControls() {
  ...
  getRootContainer().appendChild(panelContainer);
...
function buildEQWindow() {
  ...
  getRootContainer().appendChild(eqWindowContainer);
...
function buildManualWindow() {
  ...
  `;
  getRootContainer().appendChild(manualWindowContainer);
...
function buildKeyMapWindow() {
  ...
  `;
  getRootContainer().appendChild(keyMapWindowContainer);
...
function buildMIDIMapWindow() {
  ...
  getRootContainer().appendChild(midiMapWindowContainer);
...
function buildInstrumentWindow() {
  ...
  getRootContainer().appendChild(instrumentWindowContainer);
...
function buildFxPadWindow() {
  ...
  getRootContainer().appendChild(fxPadContainer);
  makePanelDraggable(fxPadContainer, fxPadDragHandle, 'ytbm_fxPadPos');
  const fxPadResizeObserver = new ResizeObserver(resizeFxPadCanvas);
  registerObserver(fxPadResizeObserver);
  fxPadResizeObserver.observe(fxPadContainer);
...
async function loadMappingsFromLocalStorage() {
  try {
    const o = await loadGlobalMappings((message, error) => reportSessionError(message, error));
...
function attachVideoMetadataListener() {
  ...
  // Attach a fresh one
  addTrackedListener(vid, "loadedmetadata", onNewVideoLoaded);
}
...
// Finally, start it once (handled by bootstrap lifecycle)
...
async function initialize() {
  try {
    ...
    addTrackedListener(document, 'click', function primeAudio() {
    ...
    hideYouTubePopups();
    addControls();
    setupPulseShowWatcher();
    setupKeyboardShortcuts();
    setupDigitCueShortcuts();
    setupStartupListeners();
    setupCueModifierShortcuts();
...
function setupMidiRandomCueIntegration() {
  if (typeof midiNotes === "undefined" || midiNotes.randomCues === undefined) return;
...
function setupMinimalUIResponsive() {
  const existing = document.getElementById("ytbm-minimal-ui-responsive");
  if (existing) return;
  const style = document.createElement("style");
...
async function bootstrap(session, store) {
  if (legacyInitialized) return;
  legacyInitialized = true;
  sessionContext = session;
  sessionAbortSignal = session.abort?.signal || null;
  appStore = store || null;
  getRootContainer();
  setupMidiRandomCueIntegration();
  setupMinimalUIResponsive();
  await initialize();
}
...
export const bootstrapLegacyApp = legacyApp.bootstrap;
export const teardownLegacyApp = legacyApp.teardown;
